...
- **Visuele pariteit bij prototype-scopes (HARDE REGEL).** Een scope met een prototype als referentie is pas klaar als de live UI de bewust getoonde interactie én vormgeving van dat prototype overneemt (flip, cards, toggles, categorie-eerst), niet alleen de functionaliteit. Waarom: anders wordt de vormgeving stilzwijgend als optioneel behandeld en ontdekt de mens de afwijking pas live.
- **Prototype-pariteit aantoonbaar maken (HARDE REGEL).** Bij een scope met een prototype als referentie benoemt de terugkoppeling expliciet welke prototype-klassen op welke live-klassen zijn toegepast (een pariteitstabel, zoals in scope 2/3/4). Wijkt de implementatie bewust af, dan staat óók die afwijking expliciet — nooit stilzwijgend weggelaten.
