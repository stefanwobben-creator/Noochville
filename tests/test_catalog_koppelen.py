...
def test_geseede_velden_tonen_als_gekoppeld_geen_inline_style(tmp_path):
    ...
    assert html.count("in catalogue") == 4                  # visitors/pageviews/visit_duration/bounce_rate geseed (A2)
...
def test_ongekoppeld_veld_toont_formulier(tmp_path):
    ...
    assert html.count("in catalogue") == 4                  # de vier geseede shopify-velden
    ...
    assert html2.count("Publish to catalogue") == 4 and "in catalogue" not in html2
...
def test_catalog_curator_ingang_open_en_niet_curator_onzichtbaar(tmp_path):
    ...
    assert "+ Link new field" in closed and "← close" not in closed
    opened = cockpit2.render_catalog(st, csrf_token="t", koppel="plausible", curator=True)
    assert "← close" in opened and "chip-opt" in opened   # sectie + scope-1-bron-picker inline
