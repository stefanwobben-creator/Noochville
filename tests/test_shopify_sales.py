...
from nooch_village.skills_impl.shopify_sales import (
    aggregate_orders, fetch_orders, ShopifySalesSkill)
...
def test_skill_failclosed_zonder_token():
...
def test_skill_met_geinjecteerde_post():
...
def test_cockpit_dashboard_render():
    ...
    shop = {"ok": True, "window_days": 28, "pairs_sold": 42, "orders": 30,
            "revenue": 5400, "currency": "EUR", "aov": 180.0,
            "by_country": [("NL", 20), ("DE", 10)], "top_products": [("Sneaker Groen", 25)]}
    page = cockpit._render_watcher_dashboard(shop)
