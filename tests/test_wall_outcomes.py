...
def test_project_lands_op_owner_met_herkomst(tmp_path):
    ...
    # herkomst als eerste systeem-entry op het NIEUWE project (bron-pid + toelichting)
    assert any(e.get("kind") == "system" and src_pid in e.get("text", "") and "want nodig" in e.get("text", "")
               for e in new.get("log", []))
    # systeem-entry op de BRON-wall (audittrail)
    assert any(e.get("kind") == "system" and "project aangemaakt" in e.get("text", "")
               and "want nodig" in e.get("text", "") for e in _log(dd, src_pid))
...
def test_action_checklist_item_met_herkomst(tmp_path):
    ...
    assert any(e.get("kind") == "system" and "omdat het moet" in e.get("text", "") for e in p.get("log", []))
...
def test_note_op_rol_met_herkomst_in_change_note(tmp_path):
    ...
    assert src_pid in notes[0].versions[0]["change_note"] and "vastleggen als kennis" in notes[0].versions[0]["change_note"]
...
def test_roloverleg_add_role_op_agenda(tmp_path):
    ...
    assert any(it["kind"] == "add_role" and "terugkerend werk" in it.get("example", "") for it in items)
...
def test_toelichting_verplicht(tmp_path):
    dd, src_pid, eid = _setup(tmp_path)
    _, msg = cockpit2.dispatch(dd, "wall_outcome",
        _form(otype="note", pid=src_pid, item=eid, note_role=_OWNER,
              content="x", toelichting="  "), username="guest")
    assert "toelichting" in msg.lower()
...
def test_systeem_entry_op_de_wall(tmp_path):
    ...
    cockpit2.dispatch(dd, "wall_outcome",
        _form(otype="note", pid=src_pid, item=eid, note_role=_OWNER,
              content="body", toelichting="mijn toelichting"), username="guest")
    sys_entries = [e for e in _log(dd, src_pid) if e.get("kind") == "system"]
    assert any("note aangemaakt" in e["text"] and "mijn toelichting" in e["text"] for e in sys_entries)
