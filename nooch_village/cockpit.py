"""⚠️ LEGACY (cockpit1). De actieve cockpit is `cockpit2.py` + `views/`; dit is het oude mens-
oppervlak (dashboard/digest/prikbord/triage-schermen) dat alleen nog door zijn eigen tests draait.
Bouw hier geen nieuwe UI. De gedeelde web-bouwstenen (`_e`/`_banner`/`_page` + basis-chrome) zijn
verhuisd naar `nooch_village/web_base.py`; nieuwe/actieve views importeren dááruit, niet hieruit.
...
from nooch_village.web_base import _e, _banner, _page  # geëxtraheerde web-basis (was hier lokaal)
...
# `_e`, `_banner`, `_page` en de basis-chrome (`_FONTS`/`_CSS`) leven nu in nooch_village.web_base
# (geëxtraheerd); hierboven staan ze geïmporteerd. Deze legacy-module gebruikt ze nog intern.
