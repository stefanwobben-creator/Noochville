...
_MBTI_HINT = "four letters, e.g. ISTP"
...
def render_inwoners(st, msg: str = "") -> str:
    ...
    tabel = (f"<table class='mtab'><tr><th>Inhabitant</th><th>MBTI</th><th>Seat(s)</th>"
             f"<th>Status</th><th class='num'>Skills</th></tr>{rijen}</table>")
    main = (f"<div class='c2-main'><h1>Inhabitants of NoochVille</h1>"
            f"<p class='muted'>The persona carries character, capacity and tooling. The mandate "
            f"(purpose, accountabilities, domains) belongs to the role and runs through governance.</p>"
            f"{_banner(msg)}<div class='card'>{tabel}</div></div>")
    return _page("Inhabitants", f"{_DS_LINK}{_nav()}<div class='c2-wrap'>{main}</div>")
...
def render_inwoner(st, persona_id: str, csrf_token: str = "", msg: str = "",
    ...
    if persona is None:
        inner = (f"{_DS_LINK}{_nav()}<div class='c2-wrap'><div class='c2-main'><h1>Inhabitant</h1>"
                 f"<div class='card'><p>This inhabitant does not exist (any more).</p>"
                 f"<p><a href='/inwoners'>← all inhabitants</a></p></div></div></div>")
        return _page("Inhabitant", inner)
    ...
    kop = (f"<h1>{_avatar(persona)} {_e(persona.name)} "
           ...
           f"<p class='muted'><a href='/inwoners'>← Inhabitants</a> · the persona carries character, "
           f"capacity and tooling; the mandate belongs to the role.</p>")
    main = (f"<div class='c2-main'>{kop}{_banner(msg)}"
            f"<div class='pgrid rov-grid'><div>{links}</div><div>{rechts}</div></div></div>")
    return _page(f"Inhabitant — {persona.name}", f"{_DS_LINK}{_nav()}<div class='c2-wrap'>{main}</div>")
...
def _personality(persona, csrf_token: str, kan_bewerken: bool, voorstellen: list) -> str:
    lees = (f"<dl class='dcol'><dt class='muted'>MBTI</dt><dd>{_e(persona.mbti or '—')}</dd>"
            f"<dt class='muted'>Instructions</dt><dd>{_e(persona.instructions or '—')}</dd>"
    ...
    form = (f"<form method='post' action='/action' class='qadd-form'>"
            ...
            f"{_field('Instructions', 'instructions', kind='textarea', value=persona.instructions, fid='f-inw-instr')}"
...
def _finetune(persona, csrf_token: str, voorstellen: list) -> str:
    ...
    de andere twee."""
    opties = [("current", persona.prompt_extra or "(empty now)")] + [
    ...
    if len(opties) == 1:
        knop = (f"<form method='post' action='/action' class='qadd-row'>"
                ...
                f"✨ Finetune with AI</button></form>")
        return (f"<div class='c2-sec'><h3>✨ Finetune with AI</h3>"
                f"<p class='muted'>The AI proposes two alternatives for your prompt extra "
                f"— tighter and looser. You choose.</p>{knop}</div>")
    rijen = ""
    ...
    return (f"<div class='c2-sec'><h3>✨ Finetune with AI</h3>"
            ...
            f"<button class='btn ok' name='action' value='persona_finetune_apply'>Use selection</button>"
            f"<button class='btn sm ghost' name='action' value='persona_finetune'>↻ new proposals</button>"
            f"</div></form></div>")


def _motor_blok() -> str:
    return ("<div class='card'><h3>No LLM</h3><p class='muted'>This is an engine: it runs "
            "on rules, not on a language model. So there is no model preference and no spend.</p></div>")
...
def _llm_blok(st, persona, csrf_token: str, kan_bewerken: bool) -> str:
    ...
    for site, model in sorted(per_taak.items()):
        ...
        bedrag = (f"€ {vak['eur']:.2f}" if vak.get("eur") else
                  ("<span class='muted'>unknown</span>" if vak.get("onbekend") else "—"))
        rijen += (f"<tr><td>{_e(site)}</td><td><code>{_e(model)}</code></td>"
                  f"<td class='num'>{bedrag}</td></tr>")
    tabel = (f"<table class='mtab'><tr><th>Task</th><th>Model</th><th class='num'>14d</th></tr>"
    ...
    if kan_bewerken:
        beheer = (f"<form method='post' action='/action' class='qadd-form'>"
                  ...
                  f"<input type='hidden' name='next' value='/inwoner?id={_e(persona.id)}'>"
                  f"<input type='hidden' name='pid' value='{_e(persona.id)}'>"
                  f"{_field('Default model', 'llm_default', value=llm.get('default', ''), fid='f-inw-llm', placeholder='gemini:gemini-2.5-flash-lite')}"
                  f"{_field('Per task (call_site=model, one per line)', 'llm_per_taak', kind='textarea', value=_pertaak_tekst(per_taak), fid='f-inw-pertaak')}"
                  f"<div class='qadd-row'><button class='btn ok' name='action' value='persona_llm'>"
                  f"Save</button></div></form>")
    return (f"<div class='card'><h3>LLM preferences</h3>"
...
def _model_cel(model: str | None) -> str:
    ...
    return f"<code>{_e(model)}</code>" if model else "<span class='muted'>village ladder</span>"
...
def _budgetbalk(besteed: float, budget: float, onbekend: int) -> str:
    ...
    staart = (f" · {onbekend} call(s) on a rung without a known price — not counted"
              if onbekend else "")
    return (f"<div class='c2-sec'><div class='muted'>LLM spend 14 days · "
            f"€ {besteed:.2f} of € {budget:.2f} budget{_e(staart)}</div>"
...
def _skills_blok(persona) -> str:
    ...
    if not (persona.skills or []):
        return ("<div class='card'><h3>Skills</h3><p class='muted'>No capacity recorded "
                "yet.</p></div>")
    rijen = "".join(
        f"<div class='rdr-row'><div><b>{_e(skill_label(s))}</b>"
        f"<div class='muted'><code>{_e(s)}</code></div></div></div>"
        for s in persona.skills)
    return (f"<div class='card'><h3>Skills <span class='muted'>— what {_e(persona.name.split()[0])} "
            f"can do (travels along to every seat)</span></h3>{rijen}"
            f"<p class='muted'>Execution still runs on the role DNA in this phase; this is the "
            f"dossier and the package manifest.</p></div>")
...
def _tools_blok(persona) -> str:
    ...
    return (f"<div class='card'><h3>Tools</h3><div class='tile-grid'>{kaarten}</div>"
            f"<p class='muted'>Tool screens belong to the inhabitant: download the package and "
            f"you get these screens with it.</p></div>")


def _zetels_blok(st, persona, zetels: list) -> str:
    if not zetels:
        return ("<div class='card'><h3>Seats</h3><p class='muted'>This inhabitant currently fills no "
                "role.</p></div>")
    blokken = ""
    for i, rec in enumerate(zetels):
        ...
        blokken += (f"<details class='c2-hist'{' open' if i == 0 else ''}>"
                    f"<summary><b>🪑 {_e(_rolnaam(rec))}</b> <span class='chip outline'>role</span></summary>"
                    f"<div class='muted'>Purpose: {_e(getattr(dna, 'purpose', '') or '—')}</div>"
                    f"<div class='c2-sec'>Required by the role → covered: {dekking or '—'}</div>"
                    f"<div class='c2-sec'><a class='btn sm' href='/node?id={_e(rec.id)}'>"
                    f"→ to the role page</a></div></details>")
    return (f"<div class='card'><h3>Seats</h3>{blokken}"
            f"<p class='muted'>The role holds the mandate (via governance). The inhabitant takes "
            f"skills and tone along on a seat change.</p></div>")
...
def _activiteit_blok(st, persona, zetels: list) -> str:
    ...
    if not events:
        return ("<div class='card'><h3>Recent activity</h3>"
                "<p class='muted'>Nothing recorded for these seats yet.</p></div>")
    rijen = ""
    for e in events:
        ...
        rijen += (f"<tr><td class='muted'>{_e(e['rol'])}</td><td>{_e(e['event'])}</td>"
                  f"<td>{doel}</td></tr>")
    return (f"<div class='card'><h3>Recent activity</h3>"
            f"<table class='mtab'><tr><th>Role</th><th>What</th><th>Where to</th></tr>{rijen}</table>"
            f"<p class='muted'>Last {len(events)} lines from the audit trail, in "
            f"file order — the bus events carry no timestamp.</p></div>")
