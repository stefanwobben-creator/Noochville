...
def _modal_html(mentions_json: str = "[]") -> str:
    ...
    return (
        ...
        "function lkRenderAll(){var mount=document.getElementById('wo-video');if(!mount||!lkRoom)return;"
        "mount.innerHTML='';var me=lkRoom.localParticipant;"
        "var parts=[me].concat(Array.from(lkRoom.remoteParticipants.values()));"
        "parts.forEach(function(p){var tile=document.createElement('div');"
        "tile.className='wo-tile'+(String(p.identity||'').indexOf('ai:')===0?' ai':'');"
        "var vt=null;p.videoTrackPublications.forEach(function(pub){if(pub.track)vt=pub.track;});"
        "if(vt){var el=vt.attach();el.className='wo-cam';el.muted=(p===me);tile.appendChild(el);}"
        "else{var off=document.createElement('div');off.className='wo-cam wo-cam-off';off.textContent='\\uD83D\\uDCF7';tile.appendChild(off);}"
        "var lbl=document.createElement('div');lbl.className='wo-tile-lbl';"
        "lbl.textContent=(p.name||p.identity||'')+(p===me?' (jij)':'');tile.appendChild(lbl);mount.appendChild(tile);});}"
        "function lkDisconnect(){if(lkRoom){try{lkRoom.disconnect();}catch(e){}}lkRoom=null;lkCircle=null;}"
        "function lkConnect(circle){var LK=window.LivekitClient;"
        "fetch('/livekit-token?circle='+encodeURIComponent(circle)).then(function(r){return r.json();}).then(function(d){"
        "if(!d.token||!d.server_url){return;}lkCircle=circle;lkRoom=new LK.Room({adaptiveStream:true,dynacast:true});"
        "['ParticipantConnected','ParticipantDisconnected','TrackSubscribed','TrackUnsubscribed','LocalTrackPublished']"
        ".forEach(function(ev){lkRoom.on(LK.RoomEvent[ev],lkRenderAll);});"
        "lkRoom.connect(d.server_url,d.token).then(function(){lkRenderAll();"
        "try{lkRoom.localParticipant.enableCameraAndMicrophone().then(lkRenderAll).catch(function(){});}catch(e){}"
        "}).catch(function(e){console.error('[werkoverleg] LiveKit connect() faalde (circle '+circle+'):',e);lkDisconnect();});"
        "}).catch(function(e){console.error('[werkoverleg] LiveKit token/join-flow faalde (circle '+circle+'):',e);});}"
        "function lkMaybe(){var vm=bd.querySelector('#wo-video[data-lk-circle]');if(!vm)return;"
        "var circle=vm.getAttribute('data-lk-circle');"
        "if(lkRoom&&lkCircle===circle){lkRenderAll();return;}"
        "if(lkRoom&&lkCircle!==circle){lkDisconnect();}"
        "lkEnsureSdk(function(){lkConnect(circle);});}"
